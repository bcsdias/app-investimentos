...
# --- Helper para Gráficos ECharts ---
def render_echarts(df, title, y_format="{value}", y_title="Valor", key=None):
    if df is None or df.empty: return
    
    # Ordenação inicial das colunas pela rentabilidade final (para a tabela)
    if not df.empty:
        last_vals = df.ffill().iloc[-1]
        sorted_cols = last_vals.sort_values(ascending=False).index
        df = df[sorted_cols]

    df_display = df.copy() # Cópia para exibir na tabela
    df_chart = df.copy()
    
    # Lógica de Porcentagem: Se o formato pede %, multiplicamos os dados por 100
    is_percent = "%" in y_format
    axis_format = "{value}"
    if is_percent:
        axis_format = "{value}%"
        # Multiplica colunas numéricas por 100 para exibição correta no eixo e tooltip
        for col in df_chart.columns:
            if pd.api.types.is_numeric_dtype(df_chart[col]):
                df_chart[col] = df_chart[col] * 100
    
    # --- OTIMIZAÇÃO DE PERFORMANCE: Downsampling ---
    MAX_POINTS = 1000
    if len(df_chart) > MAX_POINTS:
        step = len(df_chart) // MAX_POINTS
        df_resampled = df_chart.iloc[::step]
        if df_chart.index[-1] != df_resampled.index[-1]:
            df_resampled = pd.concat([df_resampled, df_chart.iloc[[-1]]])
        df_chart = df_resampled
    # -----------------------------------------------

    # Reset index para ter a data como coluna
    if df_chart.index.name is None: df_chart.index.name = 'Data'
    df_chart = df_chart.reset_index()
    x_col = df_chart.columns[0]
    
    # Datas para Eixo X (String ISO)
    if pd.api.types.is_datetime64_any_dtype(df_chart[x_col]):
        dates = df_chart[x_col].dt.strftime('%Y-%m-%d').tolist()
    else:
        dates = df_chart[x_col].astype(str).tolist()
    ...
    for col in df_chart.columns:
        ...
        # Trata NaN como None (null no JSON) para o ECharts não desenhar zero
        # Também trata Infinitos que quebram o JSON
        clean_s = df_chart[col].replace([np.inf, -np.inf], np.nan)
        values = clean_s.where(pd.notnull(clean_s), None).tolist()
        
        s_config = {
            "name": col,
            "type": "line",
            "data": values,
            "showSymbol": False, # Performance: não desenha bolinha em cada ponto
            "smooth": False,
            "emphasis": {"focus": "series"}
        }
        # Formatter específico para o tooltip desta série (adiciona %)
        if is_percent:
             s_config["tooltip"] = {"valueFormatter": JsCode("(value) => value ? value.toFixed(2) + '%' : 'N/A'").js_code}
        
        series.append(s_config)
    ...
    options = {
        ...
        "tooltip": {
            "trigger": "axis",
            "order": "valueDesc", # ORDENAÇÃO DINÂMICA DO TOOLTIP (Maior -> Menor)
        ...
        "yAxis": {"type": "value", "name": y_title, "axisLabel": {"formatter": axis_format}, "scale": True},
        "series": series,
        "dataZoom": [{"type": "inside"}, {"type": "slider", "bottom": "5%"}],
        "toolbox": {"feature": {"saveAsImage": {"title": "Salvar"}, "dataView": {"readOnly": True}, "restore": {}}}
    }
    
    st_echarts(options=options, height="450px", key=key)
    ...
    if btn_processar:
        ...
        try:
            ...
            with tab1:
                ...
                render_echarts(df_twr, "Evolução TWR (Base 100)", y_format=".1f", y_title="Base 100", key="chart_twr")
                ...
                if df_risk is not None:
                    ...
                    # Gráfico de Dispersão (Scatter) com ECharts
                    scatter_series = []
                    for idx, row in df_risk.iterrows():
                        scatter_series.append({
                            "name": row['Ativo'],
                            "type": "scatter",
                            "symbolSize": 20 if row['Ativo'] == 'Carteira' else 12,
                            "data": [[row['Volatilidade'] * 100, row['Retorno (CAGR)'] * 100, row['Sharpe']]],
                            "itemStyle": {"color": "red" if row['Ativo'] == 'Carteira' else None, "opacity": 0.8}
                        })

                    opt_scatter = {
                        "title": {"text": "Risco (Vol) x Retorno (CAGR)", "left": "center"},
                        "tooltip": {
                            "trigger": "item",
                            "formatter": JsCode("""function (params) {
                                return `<b>${params.seriesName}</b><br/>
                                        Volatilidade: ${params.data[0].toFixed(2)}%<br/>
                                        Retorno: ${params.data[1].toFixed(2)}%<br/>
                                        Sharpe: ${params.data[2].toFixed(2)}`;
                            }""").js_code
                        },
                        "xAxis": {"name": "Volatilidade (%)", "type": "value", "scale": True},
                        "yAxis": {"name": "Retorno (%)", "type": "value", "scale": True},
                        "series": scatter_series,
                        "legend": {"type": "scroll", "bottom": 0}
                    }
                    st_echarts(options=opt_scatter, height="500px", key="chart_scatter")
            ...
            with tab2:
                ...
                with col1:
                    ...
                    render_echarts(df_dd, "Drawdown", y_format=".1%", key="chart_dd")
                
                with col2:
                    st.subheader("Volatilidade Móvel (Risco)")
                    _, df_vol = report.plot_rolling_volatility(title_suffix=nome_analise, return_fig=True)
                    render_echarts(df_vol, "Volatilidade Anualizada", y_format=".1%", key="chart_vol")
                ...
                render_echarts(df_sharpe, "Sharpe Ratio", y_format=".2f", y_title="Sharpe", key="chart_sharpe")
                ...
                if modo == "Carteira DLP Invest":
                    ...
                    with col_tir:
                        ...
                        if series_tir is not None and not series_tir.empty:
                            ...
                            # Para o ECharts com formato %, dividimos por 100 para que a função render_echarts
                            # multiplique de volta e aplique a formatação correta, ou passamos direto sem %.
                            # Como render_echarts multiplica por 100 se tiver %, passamos decimal.
                            df_tir = (series_tir / 100).to_frame(name="TIR")
                            render_echarts(df_tir, "TIR Histórica", y_format=".2%", key="chart_tir")
                        else:
                    ...
                    with col_sim:
                        if simular_aportes:
                            ...
                            if df_sim is not None and not df_sim.empty:
                                render_echarts(df_sim, "Patrimônio Simulado (R$)", y_format=",.0f", y_title="R$", key="chart_sim")
                            else:
                                st.info("Dados insuficientes para simulação.")
                        else:
